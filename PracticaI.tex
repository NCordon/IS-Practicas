...
\item Existen, entre otros, los siguientes tipos de RAID: \cite{raid} \cite{imgsraid}
  \begin{itemize}
    ...
    \imagen{./images/raid-0.png}{Ejemplo de RAID 0}{raid0}{1}
    ...
    \imagen{./images/raid-1.png}{Ejemplo de RAID 1}{raid1}{1}
    ...
    \item \underline{RAID 3}: hay un disco que se usa para almacenar datos de paridades. Cada bloque en tantos trozos
    ...
    \item \underline{RAID 4}: cuando se almacena un fichero, los bloques se reparten entre los distintos discos (en este caso los bloques
    ...
    \imagen{./images/raid-5.png}{Ejemplo de RAID 5}{raid5}{1}
    ...
    en cada división cada bloque es replicado en dos discos distintos (nivel de RAID 0).
  \end{itemize}
...
\section{Cuestión 15}
\textbf{Continúe usted con el proceso de definición de RAID1 para los dos discos de 50MiB que ha creado. Muestre el 
proceso con capturas de pantalla.} 
\begin{answer}
  \begin{itemize}
   \item Debemos escoger el sistema de particiones para cada uno de los dos discos. Escogeremos MBR, tal y como se
   muestra en \ref{mbr}
   
   \imagen{./images/win/1.png}{Sistema de particiones MBR}{mbr}{0.9}
   
   \item Seleccionamos una de los volúmenes y creamos el RAID1 (New Mirrorred Volume), como se muestra en \ref{imgraid}
   
   \imagen{./images/win/2.png}{Selección de RAID 1}{imgraid}{0.9}
   
   \item Creamos el RAID 1 con los dos discos de 50 MB, asignándole un formato de archivos, que en nuestro caso ha sido
    NTFS. El proceso se muestra en
    \ref{img3}, \ref{img4} y \ref{img5}.
    
    \imagen{./images/win/3.png}{Selección de los discos del RAID}{img3}{0.9}
    
    \imagen{./images/win/4.png}{Asignación de letra de unidad}{img4}{0.9}
    
    \imagen{./images/win/5.png}{Elección del sistema de ficheros}{img5}{0.9}   
  \end{itemize}
\end{answer}
...
La referencia usada para esta pregutna ha sido: \cite{networking}

En esta pregunta tenemos dos opciones. O levantamos una conexión Ethernet nueva desde las opciones del VirtualBox
en la máquina guest, de tipo Host-only (el interface en la máquina host se llama por defecto vboxnet0) , y configuramos
las IPs de la máquina guest y host para que se hallen en la misma subred (la correspondiente al interfaz vboxnet0).

Otra alternativa es configurar una conexión Bridge Network entre la máquina host y la guest (esto es el equivalente a que
ambas máquinas estuviesen conectadas por un cable Ethernet físico), y una de tipo Internal Network para que las máquinas
virtuales pudiesen verse entre sí, de manera que los paquetes no pasasen por el host, ya que se supone que con una conexión
de tipo Bridge se tienen todas las funcionalidades que con una de tipo Internal, pero los paquetes pasan de una máquina
virtual a otra a través del host, mientras que de la otra forma pasan directamente.	
  
Se ha escogido la primera alternativa en ambas máquinas.

\subsection{Ubuntu server}
  En la imagen \ref{net} se observa cómo se hace un \texttt{ifconfig} en la máquina
  host, para averiguar la IP de \texttt{vboxnet0}, se levanta una conexión \texttt{eth1} en la máquina guest, con IP en
  la subred de \texttt{vboxnet0}, y ambas máquinas se hacen ping entre ellas. Además, se comprueba mediante \texttt{ping www.google.es}
  en el guest que se dispone de conexión a internet.

  \imagen{./images/net.jpg}{Configuración de network entre host y guest}{net}{1}
  
  La configuración final de la red se muestra en \ref{redubuntu}
  
  \imagen{./images/redubuntu.png}{Interfaces de red en Ubuntu Server}{redubuntu}{1}

\subsection{Windows Server}
  Se cambia la ip de la interfaz Ethernet 2 en Windows Server, ya que venía configurada por defecto.
  De esta forma podemos colocarla en la misma red de área local que Ubuntu Server y que el host. Se muestra en \ref{changenet}
  
  \imagen{./images/changenet.png}{Cambio IP interfaz}{changenet}{1}
  
  Se deben desactivar las reglas para dejar entrar y pasar los \texttt{ping}. Tanto en \texttt{Outbound rules} como en 
  \texttt{Inbound rules} en el Firewall de Windows. Tal como se muestra en \ref{enableping}
  
  \imagen{./images/enableping.png}{Habilitando ping en el firewall de Windows}{enableping}{1}
  
  El resultado es \ref{config}
  
  \imagen{./images/config.jpg}{Configuración de network entre host y guest}{config}{1}
