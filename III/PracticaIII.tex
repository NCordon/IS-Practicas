...
\begin{answer}1
 Antes de comenzar el proceso, introducimos un disco de 8GB en la máquina virtual (del tamaño de los dos ya presentes), ya que durante su ejecución no podemos ni añadir ni retirar discos.
En nuestro caso, tal y como refleja la \ref{rprevprev} tiene por identificador \texttt{/dev/sdc}
...
\imagen{../images/33p3.png}{Se marca el disco \texttt{/dev/sda1} como defectuoso}{r1}{1.0}
...
sudo /etc/init.d/apache2 restart
 ...
 Entramos en el navegador del host, a través de una IP que tenemos configurada en Ubuntu Server en la misma subred que el interfaz
 \texttt{vboxnet0} en la máquina host: \texttt{192.168.56.2}. Y accedemos a la página: \texttt{192.168.56.2/ganglia}

 \texttt{ganglia} nos ofrece entre otras gráficas:
 \begin{itemize}
 \item La de la carga media del servidor: esto es número de procesos encolados en el servidor en cada instante, tal y como se muestra
 en la imagen superior de \ref{load}. Se aprecia que el servidor está bastante desocupado, siendo la media del tiempo
 muestreado 0.101 procesos.
 
   \imagen{../images/64p3.jpeg}{Carga de trabajos en el servidor}{load}{.8}
 
 \item Tal y como se refleja en la \ref{net} se refleja la carga de la red: número de bytes transferidos (en azul)
 y número de bytes recibidos (en verde) por el servidor. Como podemos observar, la media de bytes enviados por segundo
 por el servidor es muy superior a la de recibidos (752 frente a 49.9)
...
\begin{answer}
 ...
 En la \ref{tablecontent} se muestra el contenido de las tablas una vez insertadas las tuplas.
 
 \imagen{../images/71p3.jpeg}{Contenido de las tablas tras la inserción}{tablecontent}{1.0}

 Queremos hallar los proveedores que suministran la pieza P1. Una forma de hacerlo sería:
 \begin{lstlisting}[style=BashInputStyle]
  mysql> select codpro from proveedor where EXISTS(select * from ventas where ventas.codpro = proveedor.codpro and ventas.codpie='P1');
 \end{lstlisting}
 
 Otra forma:
 \begin{lstlisting}[style=BashInputStyle]
  select codpro from ventas where ventas.codpie='P1';
 \end{lstlisting}

 Los resultados de profiling de ejecutar ambas sentencias se muestran en la \ref{prof1} y \ref{prof2}
  
  \imagen{../images/72p3.jpeg}{Resultados de profiling para la primera consulta}{prof1}{.8}
  \imagen{../images/73p3.jpeg}{Resultados de profiling para la segunda consulta}{prof2}{.8}
 
 Los tiempos de ejecución se muestran en \ref{timeexec}
 
 \imagen{../images/74p3.jpeg}{Tiempos de ejecución de las consultas}{timeexec}{1}
 
 La primera consulta tarda 0.00028 segundos. La segunda 0.00018425 segundos.
 
 Podemos observar que la segunda sentencia es más rápida que la primera. Veamos por qué: supongamos que hay $m$ tuplas en \texttt{ventas}
 y que hay $n$ en \texttt{proveedor}. Dado un proveedor \texttt{p}, la primera consulta recorre la tabla de \texttt{ventas} entera,
 comprobando si coinciden en una tupla el proveedor con \texttt{p} y si la pieza vendida era \texttt{P1}; por tanto
 la consulta tiene orden de eficiencia $\mathcal{O}(n\cdot m)$. La segunda consulta en cambio sólo itera por las distintas tuplas de
 \texttt{ventas}, comprobando si el código de pieza es \texttt{P1}, por lo que tiene orden de eficiencia $\mathcal{O}(m)$. De ahí
 que según la \ref{timeexec} sea 1.5 veces más rápida.
 
 En términos de sus profiling, podemos añadir que la primera consulta pasa más tiempo buscando en la caché (porque incluye una
 subconsulta). Fruto de ello hay más envíos de datos en la primera consulta que en la segunda, y se pasa más tiempo
 liberando objetos que en la segunda consulta.
